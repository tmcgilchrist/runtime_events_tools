  ...
  let env =
    Array.append
      [|
        ...
        ((* let verbose = if verbose_flag == true then "v=0x400" else "" in *)
         (* Option for setting e=runtime_events_log_wsize *)
         let log_wsize =
...
let launch_process config (exec_args : exec_config) :
    subprocess =
...
let olly config exec_args =
  ...
  Fun.protect ~finally:config.cleanup (fun () ->
      let runtime_config = { dir = config.runtime_events_dir; log_wsize = config.runtime_events_log_wsize} in
