...
let gc_stats poll_sleep json output exec_args =
  ...
  try
    olly
      { empty_config with runtime_begin; runtime_end; lifecycle; init; cleanup; poll_sleep }
      exec_args
  with Fail msg ->
    Printf.eprintf "%s\n" msg;
    exit (-1)
...
let gc_stats_cmd =
  ...
  Cmd.v info Term.(const gc_stats $ poll_sleep_option $ json_option $ output_option $ exec_args 0)
