...
module Make (Stats : Gc_stats_intf.S) = struct
  ...
  let lifecycle domain_id ts lifecycle_event _data =
    ...
    | Runtime_events.EV_RING_STOP ->
        ...
        domain_elapsed_times.(domain_id) <-
          ts -. domain_elapsed_times.(domain_id)
  ...
  let print_percentiles json output hist =
    ...
    if json then
      ...
      in
      Printf.fprintf oc
        {|{"version": 1, "wall_time": %.2f, "cpu_time": %.2f, "gc_time": %.2f, "gc_overhead": %.2f, "domain_stats": {%s}, "mean_latency": %f, "stddev_latency": %f, "min_latency": %.2f, "max_latency": %f, "distr_latency": {%s}, %s, "collections": {"minor": %i, "major": %i, "forced_major": %i, "compactions": %i}}|}
        real_time !total_cpu_time total_gc_time gc_overhead domain_stats
        mean_latency stddev_latency min_latency max_latency distribs
  ...
  let gc_stats poll_sleep json output runtime_events_dir
    ...
    let runtime_end ring_id ts phase =
      ...
      | _ -> ()
    in
    let runtime_counter ring_id ts ct value =
      Stats.runtime_counter ~ring_id ts ct value
    in
