let process_launch_failure () =
  ...
  match_raises "executable not found on path should not launch"
    ...
    (fun () -> ignore (Launch.launch_process config (Execute [ "missing.exe" ])));
  ...
  match_raises "non-executable should not launch"
    ...
    (fun () -> ignore (Launch.launch_process config (Execute [ "./run_endlessly.ml" ])));
...
let process_launch () =
  ...
  Alcotest.(check bool)
    ...
    (try
       let a = Launch.launch_process config (Execute [ "./run_endlessly.exe" ]) in
       try
